...
async def get_macro_category_for_ateco(ateco_code: str, db_session: AsyncSession) -> dict | None:
    result = await db_session.execute(select(AtecoCatalog).where(AtecoCatalog.code == ateco_code.strip()))
