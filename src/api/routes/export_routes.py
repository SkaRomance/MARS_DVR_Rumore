...
async def export_assessment_docx(
    ...
    try:
        ...
        print_settings_result = await db.execute(
        ...
        for key in section_keys:
            ...
            if template:
                sections_content[key] = template.get("content", f"<p>Sezione {key}</p>")
            else:
                sections_content[key] = f"<p>Contenuto sezione {key} - Da compilare</p>"
...
async def get_dvr_document(
    ...
    try:
        ...
        metadata = {
            ...
            "generated_at": latest_doc.created_at.isoformat() if latest_doc else None,
            "status": assessment.status,
        }
...
async def list_document_sections(
    ...
    try:
        ...
        for sec in section_definitions:
            section_content = f"<p>Sezione {sec['title']} - Contenuto da compilare</p>"
            is_modified = False
    ...
    try:
        ...
        if settings:
            ...
            if settings_data.cover_subtitle is not None:
                settings.cover_subtitle = nh3.clean_text(settings_data.cover_subtitle)
