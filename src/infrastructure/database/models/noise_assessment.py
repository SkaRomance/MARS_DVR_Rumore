...
class NoiseAssessmentResult(Base):
    ...
    job_role_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), nullable=True, index=True)

    # LEX values (Art. 190 D.Lgs. 81/2008)
    lex_8h: Mapped[float | None] = mapped_column(nullable=True, comment="LEX,8h in dB(A)")
    lex_weekly: Mapped[float | None] = mapped_column(nullable=True, comment="LEX,weekly in dB(A) - Art. 190 c.2")

    # Peak values
    lcpeak_db_c: Mapped[float | None] = mapped_column(nullable=True, comment="C-weighted peak in dB(C)")

    # Classification (Art. 188)
    risk_band: Mapped[str] = mapped_column(String(20), default="negligible")

    # Incertezza (ISO/IEC Guide 98-3)
    uncertainty_db: Mapped[float | None] = mapped_column(nullable=True, comment="Incertezza estesa in dB")
    confidence_score: Mapped[float | None] = mapped_column(nullable=True, comment="Confidence score 0-1")
